...
define systemd::unit (
  ...
  if is_string($ensure) {
    validate_re($ensure, ['^present$', '^absent$'], "systemd::unit::${name}::ensure does not match the regex.")
  } else {
  ...
  if is_string($service_type) {
    validate_re($service_type, ['^simple$', '^forking$', '^oneshot$', '^dbus$', '^notify$', '^idle$'], "systemd::unit::${name}::ensure does not match the regex.")
  } else {
    fail("systemd::unit::${name}::ensure is not a string.")
  }
  ...
  if $service_timeoutstartsec != undef {
    if is_string($service_timeoutstartsec) {
      validate_re($service_timeoutstartsec, ['^infinity$', '(([0-9]+h(our)?)?([0-9]+m(in)?)?([0-9]+s(ec)?([0-9]+ms)?)?)'], "systemd::unit::${name}::service_timeoutstartsec must match either '^infinity$' or '(([0-9]+h(our)?)?([0-9]+m(in)?)?([0-9]+s(ec)?([0-9]+ms)?)?)'")
  ...
  if $service_restartsec != undef {
    if is_string($service_restartsec) {
      validate_re($service_restartsec, ['^infinity$', '(([0-9]+h(our)?)?([0-9]+m(in)?)?([0-9]+s(ec)?([0-9]+ms)?)?)'], "systemd::unit::${name}::service_restartsec must match either '^infinity$' or '(([0-9]+h(our)?)?([0-9]+m(in)?)?([0-9]+s(ec)?([0-9]+ms)?)?)'")
